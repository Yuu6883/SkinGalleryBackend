...
function getCaller(req) {
    return `${req.method.toUpperCase()} ${req.path} HTTP/${req.httpVersion} ${req.socket.remoteAddress} ${req.headers["user-agent"]} ${req.headers["origin"] || "no origin"} (perms ${req.userPermission || "none set"})`;
}
